...
define([
    ...
    function createVerticesFromQuantizedTerrainMesh(parameters, transferableObjects) {
        ...
        var encoding = new TerrainEncoding(aaBox, hMin, maximumHeight, fromENU, hasVertexNormals, true);
        var vertexStride = encoding.getStride();
        var size = quantizedVertexCount * vertexStride + edgeVertexCount * vertexStride;
        var vertexBuffer = new Float32Array(size);
        var adjacencyList = createAdjacencyList(parameters.indices);
        var bufferIndex = 0;
        for (var j = 0; j < quantizedVertexCount; ++j) {
            var color = Color.fromHsl(0,0,0,0);
            ...
            if (hasVertexNormals) {
                ...
                var divergence = getDivergence(adjacencyList, j, positions, octEncodedNormals);
                var color = mapDivergenceToColor(divergence);
            }            

            bufferIndex = encoding.encode(vertexBuffer, bufferIndex, positions[j], uvs[j], heights[j], toPack, color);
        }
    ...
    function addSkirt(vertexBuffer, vertexBufferIndex, indexBuffer, indexBufferIndex, edgeVertices, encoding, heights, uvs, octEncodedNormals, ellipsoid, rectangle, skirtLength, isWestOrNorthEdge, exaggeration) {
        ...
        for (var i = start; i !== end; i += increment) {
            ...
            vertexBufferIndex = encoding.encode(vertexBuffer, vertexBufferIndex, position, uv, cartographicScratch.height, toPack, Color.fromHsl(0,0,0,0));
    ...
    function createAdjacencyList(indecies) {
        var al = new Map();
    ...
    var divScratchNormA = new Cartesian3();
    var divScratchNormB = new Cartesian3();
    var divScratchDp = new Cartesian3();
    var divScratchDn = new Cartesian3();

    function getDivergence(adjaceny, index, positions, octEncodedNormals) {

        function getNormal(index, scratchNormal) {
            var n = index * 2.0;
            var x = octEncodedNormals[n];
            var y = octEncodedNormals[n + 1];

            return AttributeCompression.octDecode(x, y, scratchNormal);
        }

        var sum = 0;
        var num = 0;
        var startPos = positions[index];
        var startNorm = getNormal(index, divScratchNormA);
        for (var adj of adjaceny.get(index)) {
            var adjPos = positions[adj];
            var adjNorm = getNormal(adj, divScratchNormB);

            var dp = Cartesian3.subtract(adjPos, startPos, divScratchDp);
            var dn = Cartesian3.subtract(adjNorm, startNorm, divScratchDn);
    ...
    function mapDivergenceToColor(divergence) {
        var hue = divergence < 0 ? 0 : 0.5
        var sat = Math.min(1.0, Math.abs(divergence * 16));
        return Color.fromHsl(hue, sat, 0.5, 1.0);
    }
